...
export async function POST(request: NextRequest) {
  try {
    ...
    const html = `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
        <div style="background: linear-gradient(135deg, #10b981, #059669); padding: 24px; border-radius: 12px 12px 0 0;">
          <h1 style="color: white; margin: 0; font-size: 20px;">Free Demo Request — North Star</h1>
        </div>
    ...
    await sendEmail({
      to: email,
      subject: "Free Demo Request Received — North Star",
      html,
    });
    ...
    await sendEmail({
      ...
      html: `<p>New demo request:</p><ul><li><strong>Name:</strong> ${name}</li><li><strong>Email:</strong> ${email}</li><li><strong>Phone:</strong> ${phone || "N/A"}</li><li><strong>Interest:</strong> ${programInterest || "N/A"}</li></ul>`,
      replyTo: email,
    });
